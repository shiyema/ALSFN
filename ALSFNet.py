...
from Models.BackBone.batchnorm import SynchronizedBatchNorm2d
...
class ALSFNet(nn.Module):
    def __init__(self, in_channel, n_classes, *args, **kwargs):
        super(ALSFNet, self).__init__()
        ...
        self.aspp = build_aspp(kwargs['backbone'], 16, SynchronizedBatchNorm2d, 0.5)
        self.senet = SENET()
        ...
        elif '42' in kwargs['backbone']:
            layer_channels = [ 64, 128, 256, 512]
            self.reduce_dim_in = 512
            self.reduce_dim_out = 512 // 4
        ...
        self.conv6 = nn.Sequential(
            nn.Conv2d(in_channels=512, out_channels=128, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(128),
            nn.LeakyReLU()
        )

        self.reduce_channel_b5 = nn.Sequential(
            nn.Conv2d(in_channels=512, out_channels=self.reduce_dim_out, kernel_size=1),
    ...
    def forward(self, x, *args, **kwargs):
        ...
        o, l4, l5, l6 = self.res_backbone(x)
        x = self.conv1(o)
        t = self.aspp(x)
        l4 = self.conv4(l4).view(16,128,-1)
        l5 = self.conv5(l5).view(16,128,-1)
        l6 = self.conv6(l6).view(16,128,-1)
        l = torch.cat((l4, l5, l6), dim=1)
        switch = self.senet(l)
        l4 = l4.view(16, 128, 32, 32)
        l5 = l5.view(16, 128, 32, 32)
        l6 = l6.view(16, 128, 32, 32)
        L = [l4, l5, l6]
        ...
        if 'S' in self.model_pattern:
            ...
            x_vec_s = rearrange(B, 'b c h w -> b (h w) c')
            ...
            tf_decoder_s_x = self.tf_decoder_spatial_b5(
                q_s=x_vec_s, k_s=x_s_vec_topk, v_s=None,
                #q_s=tf_encoder_s_x, k_s=None, v_s=None,
                pos_emb=pos_decoder
            )  # B (16*16) 128
        ...
        if 'C' in self.model_pattern:
            ...
            x_vec_c = rearrange(B, 'b c h w -> b c (h w)')
            ...
            tf_decoder_c_x = self.tf_decoder_channel_b5(
                q_s=x_vec_c, k_s=x_vec_c_topk, v_s=None,
                #q_s=tf_encoder_c_x, k_s=None, v_s=None,
                pos_emb=pos_decoder
            )  # B 128 (16*16)
        ...
        a = self.pixel_shuffle1(x_cat)
        b = self.double_conv1(a)
        c = self.pixel_shuffle2(b)
        x = self.double_conv2(c)
