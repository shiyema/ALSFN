...
class GetTrainTestCSV:
    ...
    def get_csv(self, pattern):
        def get_data_infos(img_path, img_format):
            ...
            for img_file in tqdm.tqdm(img_file_list):
                ...
                weight_file = img_file.replace(img_format, 'png').replace('imgs', 'weight')
                ...
                data_info['img'].append(img_file)
                data_info['label'].append(label_file)
                data_info['weight'].append(weight_file)

            return data_info

        data_information = {'img': [], 'label': [], 'weight':[]}
            ...
            if len(img_path_list) == 0:
                ...
                for img_path in img_path_list:
                    if os.path.isdir(img_path):
                        ...
                        data_information['weight'].extend(data_info['weight'])
    ...
    def get_csv_file(self, phase):
        ...
        if phase == 'seg':
            ...
            data_information['weight'] = [x.replace('img', 'weight') for x in file_list]
        elif phase == 'flow':
...
if __name__ == '__main__':
    data_path_list = [r'Data\bridge\train_samples\img']
    csv_name = 'building_test_data.csv'
    img_format_list = ['jpg']
