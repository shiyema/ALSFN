...
class ExpandImg(object):
    ...
    def __call__(self, data):
        if self.phase == 'seg':
            ...
            weight = cv2.copyMakeBorder(
                weight, int(top), int(bottom), int(left), int(right), cv2.BORDER_CONSTANT, value=0)
    ...
    def __call__(self, data):
        if self.phase == 'seg':
            ...
            if torch.rand(1) < self.prob:
                ...
                weight = weight[top:(top + crop_h), left:(left + crop_w)]
                return img, label, weight
...
class Resize(object):
    ...
    def __call__(self, data):
        if self.phase == 'seg':
            ...
            weight = cv2.resize(weight, self.size, interpolation=cv2.INTER_NEAREST)
            return img, label, weight
        elif self.phase == 'cd':
            img1, label1, img2, label2 = data
