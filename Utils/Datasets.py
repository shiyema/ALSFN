...
class Datasets(Dataset):
    ...
    def pull_item_seg(self, index):
        ...
        ori_label = np.load(label_name)
        ori_weight = np.load(weight_name)
        assert (ori_img is not None and ori_label is not None), f'{img_name} or {label_name} is not valid'

        if self.transform is not None:
            img, label, weight = self.transform((ori_img, ori_label, ori_weight))
            #img, label = self.transform((ori_img, ori_label))

        one_hot_label = np.zeros([2] + list(label.shape), dtype=float)
        ...
        return_dict = {
            ...
            'labelloss': torch.from_numpy(label),
